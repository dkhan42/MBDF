...
def density_estimate(rep,charge,size,keys,bin,bandwidth,kernel='gaussian',scaling='root'):
    """
    returns the density functions of MBDF functionals for a set of given molecules.
    """
    for k in keys:
        ii = np.where(charge == k)[0]
        if len(ii)!=0:
            return mKDE(rep[ii]/k,bin,bandwidth,kernel,scaling)
        else:
            return np.zeros(size)


def generate_DF(mbdf,nuclear_charges,n_jobs=-1,bw=0.07,binsize=0.2,kernel='gaussian'):
    """
