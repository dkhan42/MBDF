...
@numba.jit(nopython=True)
def mKDE(rep,bin,bandwidth,kernel='gaussian',scaling=False):
    """
    ...
    if scaling=='root':
        ...
        gaussian = np.exp(-(a**2)/bandwidth)
        k = (np.abs(rep)**0.5)*gaussian
        return np.sum(k,axis=1)/len(rep)
    
    else:
        a = bin.reshape(-1,1)-rep
        gaussian = np.exp(-(a**2)/bandwidth)
        
        return np.sum(gaussian,axis=1)/len(rep)


def density_estimate(reps,nuclear_charges,bin,bandwidth,kernel='gaussian',scaling='root'):
    """
    ...
    big_rep=np.zeros((reps.shape[0],len(bin)*len(keys)))
    size=len(bin)
    ...
    for i in range(len(nuclear_charges)):
        ...
        for j,k in enumerate(keys):
            ...
            if len(ii)!=0:
                arr=mKDE(reps[i][ii]/k,bin,bandwidth,kernel,scaling)
                big_rep[i,j*size:(j+1)*size]=arr
...
def generate_DF(mbdf,nuclear_charges,bw=0.07,binsize=0.2):
    """
    ...
    reps=[10*mbdf[:,:,i]/np.max(np.abs(mbdf[:,:,i])+1) for i in range(mbdf.shape[-1])]
    bin=np.arange(-10,10,binsize)
    kde=[]
    for rep in reps:
        kde.append(density_estimate(rep,nuclear_charges,bin,bw,kernel='gaussian',scaling='root'))
    return np.concatenate(kde,axis=1)
